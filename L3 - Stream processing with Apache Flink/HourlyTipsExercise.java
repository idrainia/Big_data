		...
		DataStream<Tuple3<Long, Long, Float>> hourlyTips = fares
				...
				.reduce(
						new ReduceFunction<TaxiFare>() {
							@Override
							public TaxiFare reduce(TaxiFare f1, TaxiFare f2) {
								f1.tip += f2.tip;
								return f1;
							}
						},
						new ProcessWindowFunction<TaxiFare, Tuple3<Long, Long, Float>, Long, TimeWindow>() {
							@Override
							public void process(Long driverId, Context context, Iterable<TaxiFare> elements, Collector<Tuple3<Long, Long, Float>> out) {
								TaxiFare fare = elements.iterator().next();
								out.collect(new Tuple3<>(context.window().getEnd(), driverId, fare.tip));
							}
						}
				);
